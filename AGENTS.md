...
- Модели setup-full — hook `ollama_models.yaml`; loader в `deployment/ollama_models_loader.py`, pull — `ergoms ollama_framework:pull-setup-models` (фоновый serve через `start_ollama_background` → `logs/ollama-serve.log`, не `stderr=PIPE`); в конце `setup-full` ядро останавливает serve через `host_lifecycle.yaml` → `stop_commands`
